 ...
 *
 * $Id: spull.c,v 1.10 2001/01/19 18:44:20 corbet Exp $
 *********/
...
#ifdef LINUX_24
struct block_device_operations spull_fops;
#endif
...
#ifdef LINUX_24                         
    fops:               &spull_fops,    /* Block dev operations */
#endif                                  
...
int spull_revalidate(kdev_t i_rdev)
{
    ...
    register_disk(&spull_gendisk, i_rdev, SPULL_MAXNRDEV, &spull_fops,
                    spull_size << 1);
    return 0;
}
...
#ifdef NOTNOW
    for (i = 0; i < spull_devs; i++)
            register_disk(NULL, MKDEV(major, i), 1, &spull_fops,
                            spull_size << 1);
#endif
